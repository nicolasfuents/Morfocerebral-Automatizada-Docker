#!/bin/bash
# Este script sirve para ejecutar el contenedor de FreeSurfer sobre un paciente ya procesado con recon-all
...
echo "Resultados de FreeSurfer encontrados. Ejecutando procesamiento específico de FreeSurfer..."

LOG_FS="$PACIENTE_DIR/freesurfer_log.txt"

docker run --rm \
    -v "$PACIENTE_DIR":/data/paciente:rw \
    morfocerebral:freesurfer bash -c '
        source /opt/conda/etc/profile.d/conda.sh && conda activate freesurfer_env && \
        python3 /app/main_freesurfer.py --skip_fs --dicom_dir /data/paciente/dicom' \
    2>&1 | tee "$LOG_FS"

echo "FreeSurfer (etapa personalizada) completado exitosamente para $NOMBRE_PACIENTE"

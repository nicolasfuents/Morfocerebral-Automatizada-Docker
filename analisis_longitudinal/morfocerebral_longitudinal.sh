#!/bin/bash
#morfocerebral_longitudinal.sh
# Script para procesar imágenes T1 y obtener métricas morfovolumétricas de manera automatizada
set -euo pipefail
IFS=$'\n\t'
...
procesar_paciente() {
    local FILE="$1"
    local DICOM_DIR TMP_UNZIP_DIR NOMBRE_PACIENTE FECHA_ESTUDIO PACIENTE_DIR
    local CONTAINER_NAME CONTAINER_ID LOG_PATH LOG_FSL SUBJECTS_DIR
    local SCRIPT_DIR MENSAJE_PATH DESTINATARIOS_PATH PDF_REPORTE
    ...
    if [[ -f "$FILE" && "$FILE" == *.zip ]]; then
        ...
        DICOM_DIR=$(find "$TMP_UNZIP_DIR" -type f -name "*.dcm" -exec dirname {} \; | head -n 1 || true)
        [[ -n "${DICOM_DIR:-}" ]] || { echo "[ERROR] No se hallaron DICOMs en $TMP_UNZIP_DIR"; rm -rf "$TMP_UNZIP_DIR"; return; }
        ...
        # --- NUEVO: obtener fecha del estudio ---
        FECHA_ESTUDIO=$(docker run --rm \
            -v "$DICOM_DIR":/data/dicom:ro \
            morfocerebral:utils bash -c 'python /app/extract_study_date.py /data/dicom' || true)
        FECHA_ESTUDIO=$(echo "$FECHA_ESTUDIO" | tr -cd '0-9' | cut -c1-14)
        [[ -z "$FECHA_ESTUDIO" ]] && FECHA_ESTUDIO="$(date -u +%Y%m%d%H%M%S)"
        ...
        DICOM_DIR="$FILE"
        [[ -n "$(find "$DICOM_DIR" -type f -name '*.dcm' -print -quit 2>/dev/null)" ]] || { echo "[WARN] Sin DICOMs en $DICOM_DIR"; return; }
    ...
    # --- Modificación menor para evitar colisión de contenedores ---
    CONTAINER_NAME="fs_paciente_${NOMBRE_PACIENTE//[^a-zA-Z0-9]/_}_${FECHA_ESTUDIO}"
    ...
    echo "Esperando a que finalice el contenedor $CONTAINER_NAME ($CONTAINER_ID)..."
    docker wait "$CONTAINER_ID" >/dev/null 2>&1 || true

    # logs por ID; si falla, intenta por nombre
    docker logs "$CONTAINER_ID" >> "$LOG_PATH" 2>&1 || \
    docker logs "$CONTAINER_NAME" >> "$LOG_PATH" 2>&1 || \
    echo "[WARN] No se pudieron obtener logs de $CONTAINER_NAME ($CONTAINER_ID)" >> "$LOG_PATH"

    docker rm "$CONTAINER_ID" >/dev/null 2>&1 || docker rm "$CONTAINER_NAME" >/dev/null 2>&1 || true

    echo "FreeSurfer completado."
    SUBJECTS_DIR=$(awk '/SUBJECTS_DIR configurado en:/{print $4; exit}' "$PACIENTE_DIR/dicom/logs_FS_recon_all/log.txt" 2>/dev/null || true)
    if [[ -n "${SUBJECTS_DIR:-}" ]]; then
        echo "SUBJECTS_DIR obtenido: $SUBJECTS_DIR"
    else
        echo "[WARN] No se pudo leer SUBJECTS_DIR en $PACIENTE_DIR/dicom/logs_FS_recon_all/log.txt"
    fi
